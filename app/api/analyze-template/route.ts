...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (templateFile.name.endsWith(".docx") || templateFile.name.endsWith(".doc")) {
        // For DOCX files, use a simpler approach to extract placeholders
        try {
          console.log("Processing DOCX file:", templateFile.name, "Size:", templateBuffer.byteLength)
          ...
          try {
              ...
              return NextResponse.json(
                {
                  ...
                  details: "The DOCX file is missing required content (document.xml)",
                },
                { status: 400 },
              )
            }
            ...
            return NextResponse.json(
              {
                error: "Invalid DOCX file structure",
                details: "Could not read document content: " + (e instanceof Error ? e.message : String(e)),
              },
              { status: 400 },
            )
          }
            ...
            return NextResponse.json(
              {
                ...
                details: "Your template must contain placeholders in the format {placeholder_name}",
              },
              { status: 400 },
            )
          }
          ...
          return NextResponse.json(
            {
              ...
              details: docxError instanceof Error ? docxError.message : "Invalid DOCX file",
            },
            { status: 400 },
          )
        }
      ...
      return NextResponse.json({
        ...
        fileType: templateFile.name.endsWith(".docx") || templateFile.name.endsWith(".doc") ? "docx" : "html/text",
        success: true,
      })
      ...
      return NextResponse.json(
        {
          ...
          details: extractionError instanceof Error ? extractionError.message : "Error processing template",
        },
        { status: 400 },
      )
    }
