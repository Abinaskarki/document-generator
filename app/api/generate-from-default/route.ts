      ...
      try {
        ...
        for (let index = 0; index < csvData.length; index++) {
          ...
          if (templateId === "invoice") {
            ...
            title = `Contract between ${row.party1_name || "Party 1"} and ${row.party2_name || "Party 2"}`
          } else {
...
function getRequiredFieldsForTemplate(templateId: string): string[] {
  switch (templateId) {
    case "invoice":
      return ["invoice_number", "customer_name", "item_description", "amount", "due_date"]
    case "receipt":
      return ["receipt_number", "customer_name", "item_name", "amount", "date"]
    case "contract":
      return ["party1_name", "party2_name", "contract_purpose", "start_date", "end_date"]
    default:
      return []
  }
}
