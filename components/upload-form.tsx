  ...
  const analyzeTemplate = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.details || data.error || "Failed to analyze template")
      }
      ...
      if (csvHeaders.length > 0) {
        const matches = data.placeholders.filter((p: string) => csvHeaders.includes(p))
        setMatchedFields(matches)
      }
    } catch (error) {
      console.error("Error analyzing template:", error)
      setError(`Error analyzing template: ${error instanceof Error ? error.message : "Unknown error"}`)
  ...
  const analyzeCsv = async (file: File) => {
    ...
    try {
      ...
      if (detectedPlaceholders.length > 0 && data.headers.length > 0) {
        const matches = detectedPlaceholders.filter((p) => data.headers.includes(p))
        setMatchedFields(matches)
      }
    } catch (error) {
      console.error("Error analyzing CSV:", error)
      setError(`Error analyzing CSV: ${error instanceof Error ? error.message : "Unknown error"}`)
      setCsvHeaders([])
      setMatchedFields([])
    } finally {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500">
              Upload a document with placeholders like {"{field_name}"} that will be replaced with data from your CSV.
            </p>

            {isAnalyzingTemplate && <p className="text-sm text-blue-500 mt-2">Analyzing template...</p>}

            {detectedPlaceholders.length > 0 && (
              <div className="mt-4 p-3 bg-muted rounded-md">
                <div className="flex items-center gap-2 mb-2">
                  <Info className="h-4 w-4 text-blue-500" />
                  <span className="text-sm font-medium">Detected Placeholders</span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {detectedPlaceholders.map((placeholder) => (
                    <div key={placeholder} className="text-xs bg-background px-2 py-1 rounded border">
                      {placeholder}
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-sm text-gray-500">
              Your CSV should have headers that match the placeholders in your template.
            </p>

            {isAnalyzingCsv && <p className="text-sm text-blue-500 mt-2">Analyzing CSV file...</p>}
              ...
              <div className="mt-4 p-3 bg-muted rounded-md">
                ...
                <div className="flex flex-wrap gap-2">
                  {csvHeaders.map((header) => (
                    ...
                    >
                      {header}
                      {detectedPlaceholders.includes(header) && <span className="ml-1 text-green-600">✓</span>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-green-800">
                    {matchedFields.length} of {detectedPlaceholders.length} placeholders matched with CSV headers
                  </span>
                </div>
                {matchedFields.length < detectedPlaceholders.length && (
                  <p className="text-xs text-amber-600 mt-1">
                    Warning: Some placeholders in your template don't match any CSV headers.
                  </p>
                )}
              </div>
            )}
          ...
          <Button type="submit" className="w-full" disabled={!templateFile || !csvFile || isLoading}>
